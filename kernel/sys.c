...
/*
	 创建新的会话，会话等于一个或多个进程组的集合，进程将脱离原来的会话和终端，
	 因为终端对应唯一的会话,如果是领头进程，则出错,因为该进程是零头，则旧的会话中存储一个组id等于该进程id，
	 新建会话后，该进程对应的组会存着另外一个会话中，与存在旧会话矛盾，
	 否则该进程成为会话的领头，会话id组id等于该进程的id，并且脱离终端
*/
